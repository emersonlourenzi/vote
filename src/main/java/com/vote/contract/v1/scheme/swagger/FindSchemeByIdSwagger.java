...
@ApiOperation(value = "Busca pauta por id", response = AssociateContractResponse.class,
    responseContainer = "Mono", produces = "application/json")
@ApiResponses({
    @ApiResponse(code = 200, message = "Busca concluida com sucesso"),
    @ApiResponse(code = 400, message = "Bad request"),
    @ApiResponse(code = 404, message = "Pauta não encontrada"),
    @ApiResponse(code = 409, message = "Conflito na busca da pauta"),
    @ApiResponse(code = 500, message = "Internal error", response = Error.class)
})
public @interface FindSchemeByIdSwagger {
}
